from model.ResNet.Res50 import Res50_FPN
...
import torch
import numpy as np
from model.BinarizedModule import BinarizedModule
from torchvision import models
from model.PreciseRoIPooling.pytorch.prroi_pool import PrRoIPool2D
...
from misc.get_bbox import get_boxInfo_from_Binar_map
from misc.layer import Gaussianlayer, Point2Mask
from .optimal_transport_layer import Optimal_Transport_Layer
from .feature_transformer import FeatureTransformer,AttentionalGNN, KeypointEncoder,normalize_keypoints
from model.points_from_den import *
from collections import  OrderedDict
import copy
class Video_Crowd_Counting(nn.Module):
    def __init__(self, cfg, cfg_data, pretrained=True):
        super(Video_Crowd_Counting, self).__init__()
        self.cfg = cfg
        ...
        if cfg.NET == 'Res50_FPN':
            self.Extractor = Res50_FPN(pretrained=pretrained)
        elif cfg.NET == 'HR_Net':
            self.Extractor = get_seg_model()
        elif cfg.NET == 'VGG16_FPN':
        ...
        # self.feature_transformer = FeatureTransformer(FT_config).cuda()
        ...
        # self.position_encoder = KeypointEncoder(
        #     OT_config['feature_dim'], OT_config['keypoint_encoder']).cuda()
        # self.gnn = AttentionalGNN( OT_config['feature_dim'], OT_config['GNN_layers']).cuda()

        # self.final_proj = nn.Conv1d(
        #     OT_config['feature_dim'], OT_config['feature_dim'],
        #     kernel_size=1, bias=True).cuda()

        # self.counter = VGG16_FPN()
        # self.counter = torch.nn.DataParallel(self.counter).cuda()
        # state_dict = torch.load(cfg.PRE_VGG_WEIGHTS)
        # new_state_dict = OrderedDict()
        # for k, v in state_dict.items():
        #     name = k[10:]
        #     new_state_dict[name] = v
        # self.counter.load_state_dict(new_state_dict,strict=False)
        # self.counter.eval()
    @property
    def loss(self):
    ...
    def val_forward(self,img,target,frame_signal):
        ...
        match_gt, pois = self.get_ROI_and_MatchInfo(target[0], target[1],noise='ab')
    ...
    def test_forward(self, img, frame_signal):
        ...
        if (np.array(count_in_pair) > 0).all():
            ...
            sim_matrix =sim_matrix.squeeze(0).cpu().numpy()
            sim_matrix = sim_matrix / 256 ** .5
            # scores, indices0, indices1, mscores0, mscores1 = self.Matching_Layer( mdesc0, mdesc1)
            # import  pdb
            # pdb.set_trace()
            match_matrix = np.zeros(sim_matrix.shape, dtype=bool)

            tp, pre_inflow, pre_outflow, tp_pred_index, tp_gt_index = compute_metrics(sim_matrix, match_matrix, mdesc0.size(0), 12.5)
            ...
            pre_inflow = count_in_pair[1].item()
        matched_results = {
            'matches0': tp_pred_index,  # use -1 for invalid match
            'matches1': tp_gt_index,  # use -1 for invalid match
...
from .MatchTool.utils import hungarian
def compute_metrics(dist_matrix,match_matrix,pred_num,sigma):
    for i_pred_p in range(dist_matrix.shape[0]):
    ...
    outflow = fp_pred_index.shape[0]
    inflow = fn_gt_index.shape[0]

    # # #
    # import  pdb
    # pdb.set_trace()
    return tp,inflow,outflow,tp_pred_index, tp_gt_index
