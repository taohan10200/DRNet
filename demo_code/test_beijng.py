...
parser.add_argument(
    '--DATA_PATH', type=str, default='/data/GJY/ht/CVPR2022/dataset/demo_video',
    help='Directory where to write output frames (If None, no output)')
parser.add_argument(
    '--output_dir', type=str, default='/data/GJY/ht/CVPR2022/dataset/demo_video/pred',
    help='Directory where to write output frames (If None, no output)')
parser.add_argument(
    '--test_intervals', type=int, default=90,
    help='Directory where to write output frames (If None, no output)')
parser.add_argument(
...
# parser.add_argument(
#     '--model_path', type=str,
#     default='./exp/HT21/11-06_01-21_HT21_VGG16_FPN_5e-05(no_hard)/ep_6_iter_15000_mae_12.805_mse_13.526_seq_MAE_31.993_WRAE_36.773_MIAE_5.090_MOAE_6.144.pth',
#     help='pretrained weight path')
...
def test(cfg_data):
    ...
    sub_dataset = TestDataset(scene_name='beijingxi',
            ...
            with torch.no_grad():
                ...
                if kpts0.shape[0] > 0 and kpts1.shape[0] > 0:
                    save_visImg(kpts0, kpts1, matches, confidence, vi, img[0].clone(), img[1].clone(),
                                opt.test_intervals, opt.output_dir, time, cnt, scene_id, scores,pred_dict,restore_transform)
...
def save_visImg( kpts0, kpts1, matches, confidence, vi, last_frame, cur_frame, intervals,
                save_path, time=None, cnt=None, scene_id='', scores = None, pred_dict =None, restore_transform=None):
    valid = matches > -1
    ...
    number = len(pred_dict['outflow'])
    # for  i in range(number - number%intervals, number):
    #     outflow_cnt +=(pred_dict['outflow'][i] - pred_dict['outflow'][i-1])
    ...
    plt.xlim(0, 200)
    plt.ylim(0, 1000)
    plt.text(3,830, r"Pedestrian number: $N= N_{0} + \sum N_{in}$", fontsize=32,fontfamily='Times New Roman')
